...
fun! s:ApplyPostFilter() "{{{
    ...
    if filter isnot g:EmptyFilter
        ...
        " TODO refine this
        let [ rc, hadBuilt ] = s:EvalPostFilter( filter, typed, leader )
        ...
        if rc is s:DONE
            ...
            if filter.rst.toBuild
                ...
                if 0 > buildrc
                    return [ s:Crash(), filter.rst.toBuild ]
                endif
...
fun! s:EvalPostFilter( filter, typed, leader ) "{{{
    ...
    let a:filter.rst.toBuild = 0
    if has_key( a:filter.rst, 'action' )
        let frst = a:filter.rst

        if frst.action == 'build'
            let frst.toBuild = 1
        ...
        if act.name == 'build'
            if has_key( a:filter, 'text' )
                ...
                let frst.text = a:typed
            end

        elseif frst.action ==# 'keepIndent'
            let frst.nIndent = 0

        elseif frst.action =~# '\vembed'
...
fun! s:HandleDefaultValueAction( filter ) "{{{
    ...
    elseif frst.name ==# g:XPTact.build
        " same as 'embed'
        return s:EmbedSnippetInLeadingPlaceHolder( rctx, a:filter.rst.text )
    ...
    elseif frst.action ==# g:XPTact.removePH
        ...
        if has_key( a:filter.rst, 'text' )
            let postaction = s:FillinLeadingPlaceHolderAndSelect( rctx, a:filter.rst.text )
        endif
        if x.renderContext.processing
...
fun! s:FillinLeadingPlaceHolderAndSelect( ctx, str, flt_rst ) "{{{
    ...
    " TODO use new-fashion runtime build
...
fun! s:ApplyDefaultValueToPH( filter ) "{{{
    ...
    call xpt#flt#Eval( a:filter, rctx.ftScope.funcs, { 'startPos' : start } )
    ...
    if has_key( a:filter.rst, 'action' )
        ...
        if a:filter.rst.action == 'pum'
            " popup
            return s:DefaultValuePumHandler( rctx, a:filter )

        elseif a:filter.rst.action == 'complete'
            " complete pum
            let rc = s:DefaultValueShowPum( rctx, a:filter )
            return rc

        else

            let rc = s:HandleDefaultValueAction( a:filter )
            return ( rc is -1 )
                  \ ? s:FillinLeadingPlaceHolderAndSelect( rctx, '' )
                  \ : rc
        endif

    elseif has_key( a:filter.rst, 'text' )
        " string
        return s:FillinLeadingPlaceHolderAndSelect( rctx, a:filter.rst.text )

    else
        return s:FillinLeadingPlaceHolderAndSelect( rctx, '' )
...
fun! s:DefaultValuePumHandler( renderContext, flt_rst ) "{{{
    ...
    elseif pumlen == 1
        return s:FillinLeadingPlaceHolderAndSelect( a:renderContext, a:filter.rst.pum[0] )
